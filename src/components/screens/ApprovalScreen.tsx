...
const TYPE_CONFIG: Record<string, { label: string; icon: string; color: string; bg: string }> = {
  shift_swap: { label: 'Đổi ca', icon: 'swap_horiz', color: '#2563eb', bg: '#eff6ff' },
  time_off: { label: 'Nghỉ phép', icon: 'event_busy', color: '#d97706', bg: '#fffbeb' },
  overtime: { label: 'Tăng ca', icon: 'schedule', color: '#059669', bg: '#ecfdf5' },
  other: { label: 'Khác', icon: 'more_horiz', color: '#7c3aed', bg: '#f5f3ff' },
};
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          ...
          <div
            ...
            style={{ backgroundColor: stat.bg, borderColor: `${stat.color}22` }}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${
              filter === f.key
                ...
                : 'bg-white text-[#454652] border border-[#c6c5d4] hover:bg-[#f5f2fb]'
            }`}
          >
            {f.label}
            {f.key === 'pending' && pendingCount > 0 && (
              <span className="ml-1.5 bg-[#d97706] text-white text-xs px-1.5 py-0.5 rounded-full">
                {pendingCount}
              </span>
            )}
          </button>
      ...
      <div className="relative mb-5">
        <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#767683]">
          search
        </span>
        <input
          type="text"
          ...
          className="w-full pl-10 pr-4 py-3 rounded-xl bg-white border border-[#c6c5d4] text-sm focus:outline-none focus:ring-2 focus:ring-[#6e6eff] focus:border-transparent"
        />
      </div>
      ...
      <div className="flex flex-col gap-3">
        {filtered.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-2xl border border-[#c6c5d4]/40">
            ...
            <p className="text-[#767683] font-medium">Không có yêu cầu nào</p>
          </div>
        ) : (
          filtered.map((req) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#767683] mt-0.5">{formatDate(req.dateString)}</p>
                    </div>
                  </div>
                  <span
                ...
                {/* Title & Description */}
                <h3 className="font-bold text-sm text-[#1b1b21] mb-1">{req.title}</h3>
                <p className="text-xs text-[#454652] leading-relaxed mb-3">{req.description}</p>

                {/* Target employee if swap */}
                {req.targetEmployee && (
                  <div className="flex items-center gap-2 mb-3 bg-[#f5f2fb] rounded-lg px-3 py-2">
                    <span className="material-symbols-outlined text-sm text-[#6e6eff]">
                      person_swap
                    </span>
                    <span className="text-xs text-[#454652]">
                      Đổi với:{' '}
                      <span className="font-semibold text-[#000666]">{req.targetEmployee}</span>
                    </span>
                  </div>
                )}
                ...
                {req.managerNote && (
                  <div className="bg-[#f5f2fb] rounded-lg px-3 py-2 mb-3">
                    ...
                    <p className="text-xs text-[#454652]">{req.managerNote}</p>
                  </div>
                )}

                {/* Action buttons (only for pending) */}
                {req.status === 'pending' && currentUser.role === 'manager' && (
                  <div className="flex gap-2 pt-2 border-t border-[#f0eff4]">
                    <button
                      onClick={() => handleOpenModal(req, 'reject')}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-medium bg-[#fee2e2] text-[#dc2626] hover:bg-[#fecaca] transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-medium bg-[#d1fae5] text-[#059669] hover:bg-[#a7f3d0] transition-colors"
                    >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            >
              <span
                ...
                style={{ color: modalAction === 'approve' ? '#059669' : '#dc2626' }}
              >
                {modalAction === 'approve' ? 'check_circle' : 'cancel'}
              </span>
              <h3 className="font-headline text-lg font-bold text-[#1b1b21]">
            ...
            <div className="flex gap-3 p-5 pt-0">
              <button
                ...
                className="flex-1 py-3 rounded-xl text-sm font-medium bg-[#f5f2fb] text-[#454652] hover:bg-[#eae7ef] transition-colors"
              >
                Hủy
              </button>
              <button
