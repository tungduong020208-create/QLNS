  ...
  return (
    <div className="pb-28 pt-20 px-4 max-w-3xl mx-auto w-full antialiased">
      ...
      <div className="flex justify-between items-center mb-5">
        <div>
          <h2 className="font-heading text-2xl md:text-3xl font-bold text-[#0F1E44]">
            Báo cáo tổng hợp
          </h2>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
            period === 'day'
              ...
              : 'bg-[#F5EDDF] text-[#0F1E44] border border-[#E8DFD0] hover:bg-[#E8DFD0]'
          }`}
        >
          Ngày
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5 mb-6">
        {/* KPI 1 */}
        <div className="bg-white border border-[#E8DFD0]/60 rounded-2xl p-5 shadow-sm flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[#7A829A]">
            <span className="material-symbols-outlined text-[20px] text-[#0F1E44]">collections</span>
            <h3 className="text-xs font-semibold uppercase tracking-wide">Tổng minh chứng</h3>
          </div>
          <div className="font-heading text-3xl font-bold text-[#0F1E44]">
            {totalEvidencesCount.toLocaleString('vi-VN')}
          </div>
          <div className="text-[11px] font-semibold text-[#0F1E44] bg-[#F5EDDF] py-1 px-2.5 rounded-full w-max mt-auto flex items-center gap-1">
        ...
        <div className="bg-white border border-[#E8DFD0]/60 rounded-2xl p-5 shadow-sm flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[#7A829A]">
            <span className="material-symbols-outlined text-[20px] text-[#0F1E44]">thumb_up</span>
            <h3 className="text-xs font-semibold uppercase tracking-wide">Tỷ lệ Tốt</h3>
          </div>
          <div className="font-heading text-3xl font-bold text-[#0F1E44]">
            {goodRate}%
          </div>
          <div className="w-full bg-[#F5EDDF] h-2 rounded-full mt-2 overflow-hidden">
            <div
              className="bg-[#0F1E44] h-full rounded-full transition-all duration-500"
        ...
        <div className="bg-white border border-[#E8DFD0]/60 rounded-2xl p-5 shadow-sm flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[#7A829A]">
            <span className="material-symbols-outlined text-[20px] text-[#0F1E44]">star_rate</span>
            <h3 className="text-xs font-semibold uppercase tracking-wide">Tổng điểm đã cấp</h3>
          </div>
          <div className="font-heading text-3xl font-bold text-[#0F1E44]">
            {totalPointsGiven.toLocaleString('vi-VN')}
          </div>
          <div className="text-[11px] text-[#7A829A] bg-[#F5EDDF] py-1 px-2.5 rounded-full w-max mt-auto font-medium">
      ...
      {/* Main Chart Area */}
      <div className="bg-white border border-[#E8DFD0]/60 rounded-2xl p-5 shadow-sm mb-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h3 className="font-heading text-lg font-bold text-[#0F1E44]">Xu hướng đánh giá</h3>
            <p className="text-xs text-[#7A829A]">Tỷ lệ đạt chuẩn qua các tuần trong tháng</p>
          </div>
          <button
            onClick={() => setSelectedWeek(WEEKLY_CHART_DATA[2])}
            className="text-[#0F1E44] text-xs font-bold flex items-center gap-1 hover:underline cursor-pointer"
          >
        ...
        <div className="h-48 flex items-end justify-between gap-3 sm:gap-6 mt-4 px-2 sm:px-4 relative">
          ...
          <div className="absolute inset-0 flex flex-col justify-between pointer-events-none pb-6">
            <div className="w-full border-b border-[#F5EDDF] h-0" />
            <div className="w-full border-b border-[#F5EDDF] h-0" />
            <div className="w-full border-b border-[#F5EDDF] h-0" />
            <div className="w-full border-b border-[#F5EDDF] h-0" />
          </div>
              ...
              >
                ...
                <div
                  className={`w-full max-w-[64px] rounded-t-lg transition-all duration-300 ${
                    isHighlighted
                      ? 'bg-[#0F1E44] shadow-[0_0_14px_rgba(0,6,102,0.35)] ring-2 ring-[#0F1E44]/30'
        ...
        {/* X-Axis labels */}
        <div className="flex justify-between mt-3 text-[#7A829A] text-xs font-semibold px-2 sm:px-4 border-t border-[#F5EDDF] pt-2">
          {WEEKLY_CHART_DATA.map((item) => (
            <span
              key={item.week}
              onClick={() => setSelectedWeek(item)}
              className={`cursor-pointer transition-colors ${
                selectedWeek?.week === item.week || item.isCurrent ? 'text-[#0F1E44] font-bold' : ''
              }`}
            >
        ...
        {selectedWeek && (
          <div className="mt-4 p-3 bg-[#f5f2fb] rounded-xl border border-[#E8DFD0]/40 flex items-center justify-between text-xs text-[#0F1E44]">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[#0F1E44]">insights</span>
              <span>
                <strong>{selectedWeek.week}:</strong> {selectedWeek.goodCount}/{selectedWeek.totalCount} minh chứng TỐT ({selectedWeek.percentage}%)
              </span>
            </div>
            <span className="font-bold text-[#0F1E44]">+{selectedWeek.points} điểm</span>
          </div>
        )}
      </div>
      ...
      {/* Attention List */}
      <div className="mb-8">
        <h3 className="font-heading text-lg font-bold text-[#0F1E44] mb-3 flex items-center gap-2">
        ...
        <div className="flex flex-col gap-3">
          {ATTENTION_EMPLOYEES.map((emp) => (
            ...
            >
              {emp.avatar ? (
                <img
                  className="w-11 h-11 rounded-full object-cover border border-[#E8DFD0]"
                  src={emp.avatar}
                  alt={emp.name}
                />
              ) : (
                <div className="w-11 h-11 rounded-full bg-[#F5EDDF] flex items-center justify-center font-heading text-base font-bold text-[#0F1E44]">
      ...
      <div className="flex justify-center mb-6">
        <button
          ...
          className="bg-[#0F1E44] text-white text-sm font-bold px-6 py-3 rounded-xl shadow-[0_4px_12px_rgba(0,6,102,0.18)] hover:bg-[#1a237e] transition-all flex items-center gap-2 active:scale-95 cursor-pointer"
        >
      ...
      {showExportModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-[#E8DFD0] animate-in fade-in zoom-in-95 duration-150">
            <div className="w-12 h-12 rounded-xl bg-[#dee0ff] text-[#0F1E44] flex items-center justify-center mb-3">
              <span className="material-symbols-outlined text-[28px]">file_download_done</span>
            </div>
            <h3 className="font-heading text-lg font-bold text-[#0F1E44] mb-1">Xuất file thành công!</h3>
            ...
            <button
              ...
              className="w-full py-2.5 bg-[#0F1E44] text-white rounded-xl text-xs font-bold hover:bg-[#1a237e]"
            >
              Đã hiểu
            </button>
          </div>
      ...
      {showCustomDateModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-[#E8DFD0] animate-in fade-in zoom-in-95 duration-150">
            <h3 className="font-heading text-lg font-bold text-[#0F1E44] mb-3">Chọn khoảng thời gian</h3>
            <div className="space-y-3 mb-4 text-xs">
              <div>
                <label className="block font-semibold text-[#7A829A] mb-1">Từ ngày:</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block font-semibold text-[#7A829A] mb-1">Đến ngày:</label>
                <input
                  type="date"
                  value={customEndDate}
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="px-4 py-2 text-xs font-semibold text-[#7A829A] hover:bg-[#f5f2fb] rounded-lg"
              >
                Hủy
              </button>
              <button
                onClick={() => setShowCustomDateModal(false)}
                className="px-4 py-2 text-xs font-semibold bg-[#0F1E44] text-white rounded-lg hover:bg-[#1a237e]"
              >
                Áp dụng
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-[#E8DFD0] animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center gap-3 mb-4">
              {selectedEmployeeAlert.avatar ? (
                ...
                <div className="w-12 h-12 rounded-full bg-[#F5EDDF] flex items-center justify-center font-bold text-[#0F1E44]">
            ...
            <div className="p-3 bg-[#ffdad6]/60 border border-[#ba1a1a]/30 rounded-xl text-xs mb-4">
              ...
              <p className="text-[#7A829A]">{selectedEmployeeAlert.notes}</p>
            </div>

            <div className="flex gap-2">
              <button
                onClick={() => setSelectedEmployeeAlert(null)}
                className="flex-1 py-2 text-xs font-semibold text-[#7A829A] hover:bg-[#f5f2fb] rounded-lg border border-[#E8DFD0]"
              >
                Đóng
              </button>
              <button
